/* Программа калькулятор для выполнения математических операций.
 ...
 - Далее вводятся число(а) над которым(и) будет произведена выбранная математическая операция
 */
...
while (true) // пока истина. бесконечный цикл, далее сделано условие для выхода из него
{
    ...
    else if (command == 5)
    {
        ...
        {
            Console.WriteLine("\n###################\n" +
                              $"Результат: {(double)1 / result}" + // вывод результата для отрицательной степени
    ...
    else if (command == 8)
    {
        ...
        int result = 1;
        for (int i = 1; i <= num1; i++) // цикл с предусловием. считает факториал
        {
